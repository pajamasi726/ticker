...
export function StatusWall({ services, onSelect, stale = false }: { services: ServiceView[]; onSelect?: (id: string) => void; stale?: boolean }) {
  const t = useT()
  ...
  return (<div className={`wall${stale ? ' wall--stale' : ''}`}>{sorted.map((s) => <ServiceTile key={s.id} service={s} onSelect={onSelect} />)}</div>)
}
