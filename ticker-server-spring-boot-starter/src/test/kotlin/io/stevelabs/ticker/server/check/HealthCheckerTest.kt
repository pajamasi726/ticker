...
class HealthCheckerTest {
    ...
    private val props = PollProperties(timeout = Duration.ofMillis(500), degradedLatencyMs = 100L)
    ...
    @Test fun `http 2xx is SUCCESS`() {
        ...
        val r = HttpHealthChecker(restClient, props).check(target(ServiceType.HTTP))
    ...
    @Test fun `spring actuator UP is SUCCESS`() {
        ...
        val r = SpringHealthChecker(restClient, props).check(target(ServiceType.SPRING))
        assertThat(r.outcome).isEqualTo(CheckOutcome.SUCCESS)
    }
    ...
    @Test fun `supports matches type`() {
        assertThat(HttpHealthChecker(restClient, props).supports(ServiceType.HTTP)).isTrue()
        assertThat(SpringHealthChecker(restClient, props).supports(ServiceType.SPRING)).isTrue()
    }
}
